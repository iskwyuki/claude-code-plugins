# claude-code-plugins セットアップ手順

この md ファイルは Plugin install 時に `~/.claude/plugins/claude-code-plugins/SETUP.md` に配置され、`/claude-code-plugins:bootstrap` skill から参照されます。
...
配信対象の全体は `~/.claude/plugins/claude-code-plugins/assets/` で確認できます。
...
```
/plugin marketplace remove iskwyuki/claude-code-plugins
/plugin marketplace add iskwyuki/claude-code-plugins
/plugin install claude-code-plugins@iskwyuki
```
