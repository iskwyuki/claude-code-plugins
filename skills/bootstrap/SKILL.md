---
name: bootstrap
description: iskwyuki-claude-plugins の初回セットアップ skill。Plugin install 直後に /iskwyuki-claude-plugins:bootstrap で呼び出し、SETUP.md を提示してからプロジェクトの .claude/ に全 asset を初回展開する。
---
...
```
cat ~/.claude/plugins/iskwyuki-claude-plugins/SETUP.md
```
...
プロジェクトルートの `.claude/<type>/` と配信元 `~/.claude/plugins/iskwyuki-claude-plugins/assets/<type>/` の差分を表示する。
